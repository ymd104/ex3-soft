...
type exval =
  ...
  | ProcV2 of id * exp * dnval Environment.t ref
and dnval = exval
...
let rec eval_exp env = function
  ...
  | DFunExp (id, exp) -> ProcV2 (id, exp, ref env)

     (*
     let newenv = Environment.extend id (eval_exp env exp) env in
     ProcV (id, exp, ref newenv)
     *)
  ...
  | AppExp (exp1, exp2) ->
    ...
    let arg = eval_exp env exp2 in
      ...
      | ProcV2 (id, body, env') ->
        let newenv' = Environment.merge env !env' in
        let newenv = Environment.extend id arg newenv' in
        eval_exp newenv body
